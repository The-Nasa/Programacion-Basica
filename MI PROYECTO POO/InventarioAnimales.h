...
// Declaración anticipada de la clase InventarioA
class InventarioA;
...
public:
    ...
    // Función amiga para solicitar el estado del animal
    friend void solicitarEstado(Animal& animal);

    // Función amiga para acceder al estado del animal
...
public:
    void mostrarInventarioAnimales() {
        ...
        } // Asume que ya se limpió el buffer anteriormente.
    }
   ...
   void agregarAnimal() {
    ...
    if (inventarioAnimales.empty()) {
        cout << "El inventario de animales está vacío." << endl;
        return;
    }
    ...
    for (auto it = inventarioAnimales.begin(); it != inventarioAnimales.end(); ) {
        if (it->especie == especie && it->getEstado() == estado) {
            ...
            return; // Sal de la función después de manejar el animal especificado.
        } else {
            ++it;
        }
    }
...
// Definición de la función para solicitar el estado del animal
void solicitarEstado(Animal& animal) {
    cout << "Ingrese el estado del animal (saludable, enfermo, herido, etc.): ";
    cin >> animal.estado;
}

// Definición de la función para obtener el estado del animal
...
private:
    ...
    string descripcion;  // Descripción del movimiento (nombre del objeto y cantidad)
...
private:
    string tipo;  // Tipo de movimiento (agregar o quitar)
    string descripcion;  // Descripción del movimiento (nombre del animal y cantidad)
